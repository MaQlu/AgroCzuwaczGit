...
unsigned long pumpDuration = 7000;       // Domyślny czas działania pompy w milisekundach
...
void sendSensorData() {
  ...
  String json = "{\"temperature\":" + String(data.temp, 1) +
               ...
               ",\"pumpDuration\":" + String(pumpDuration / 1000) + // czas w sekundach
...
void handleWaterPlant() {
  ...
  // Logowanie
  Serial.println("[PUMP] Manual watering activated for " + String(pumpDuration) + "ms");

  // Response
  String response = "{\"status\":\"success\",\"message\":\"Watering completed\",\"duration_ms\":" + String(pumpDuration) + "}";
  server.send(200, "application/json", response);
}
...
void handleDebugPump() {
  ...
  // Logowanie
  Serial.println("[DEBUG-PUMP] Force activated for " + String(pumpDuration) + "ms");

  // Prosta odpowiedź JSON
  String response = "{\"status\":\"success\",\"message\":\"Debug pump activated\",\"duration_ms\":" + String(pumpDuration) + "}";
  server.send(200, "application/json", response);
}
...
void handleSetPumpDuration() {
  ...
  unsigned long newPumpDuration;
  
  if (server.hasArg("duration")) {
    // Form-urlencoded
    newPumpDuration = server.arg("duration").toInt() * 1000; // Konwersja sekund na milisekundy
  } else {
    // Raw JSON
    String body = server.arg("plain");
    if (body.length() == 0) {
    ...
    newPumpDuration = doc["duration"].as<unsigned long>() * 1000; // Konwersja sekund na milisekundy
  }

  if (newPumpDuration < 1000 || newPumpDuration > 60000) { // Walidacja zakresu (1-60 sekund)
    server.send(400, "application/json", "{\"status\":\"error\",\"message\":\"Duration must be between 1 and 60 seconds\"}");
    return;
  }

  unsigned long oldPumpDuration = pumpDuration;
  pumpDuration = newPumpDuration;

  // Logowanie zmiany do Serial Monitora
  Serial.println("Zmieniono czas działania pompy z " + String(oldPumpDuration / 1000) + 
                 " sekund na " + String(pumpDuration / 1000) + " sekund");

  String response = "{\"status\":\"success\",\"oldDuration\":" + String(oldPumpDuration / 1000) +
                    ",\"newDuration\":" + String(pumpDuration / 1000) + "}";
  server.send(200, "application/json", response);
}
// ========== SETUP I LOOP ==========
void setup() {
